...
for img_file in tqdm(glob.glob(os.path.join(images_path, "*.tif"))):
    ...
    tiles = torch.nn.functional.interpolate(tiles, size=(config.image_size, config.image_size))
    with torch.no_grad():
    ...
    prediction2 = einops.rearrange(prediction2, "(tile_h tile_w) h w -> (tile_h h) (tile_w w)", tile_h=1, tile_w=1)
